...
# Create an ExcelFile object first
    ...
    sheet_names=[normalize_and_replace(i) for i in sheet_names]
...
def condition_check(df1,state_name):
    name_exist_in_sheet_for_compare=str(state_name).strip()
    condition = df1.iloc[:, 0].str.strip() == name_exist_in_sheet_for_compare
    ...
    for xls_path in glob.glob(state_path_xls)+glob.glob(state_path_xlsx):
        ...
        if sho==1:
            ...
            if name_exist_in_sheet=="":
                ...
                print(f"\033[34m{alram_string}\033[0m")
